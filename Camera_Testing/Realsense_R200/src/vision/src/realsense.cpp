...
void color(const sensor_msgs::Image::ConstPtr& msg)
{
    ...
    //cv::Mat color_img;
    //cv::cvtColor(color_img, img_color->image, CV_RGB2BGR);
    //cv::imshow("color", color_img);

    //std::cout<< "image type" << img_color->image.type() << std::endl;
...
int main(int argc, char* argv[])
{
    ...
    ros::Subscriber sub1 = nh.subscribe("/camera/depth/image_raw", 1, depth);

    ros::Subscriber sub2 = nh.subscribe("/camera/color/image_raw", 1, color);
